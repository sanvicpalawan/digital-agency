...
export const priorityRank: Record<WorkstationPriority, number> = { urgent: 0, moderate: 1, downtime: 2 };
...
export function PriorityBadge({ priority, className }: { priority: WorkstationPriority; className?: string }) {
  return (
    <span
      className={cn(
...
export function Stamp({ iso, className }: { iso: string; className?: string }) {
  return (
    <span
      ...
      className={cn('inline-flex items-center gap-1 text-[11px] text-slate-500 dark:text-white/40 whitespace-nowrap', className)}
    >
...
export function AuthorChip({ name, iso, action }: { name: string; iso: string; action?: ReactNode }) {
  return (
  ...
  return (
    <section className="rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50/60 dark:bg-white/[0.03] p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        {typeof count === 'number' && (
          <span className="ml-auto text-[11px] font-semibold text-slate-400 dark:text-white/30">{count}</span>
        )}
      </div>
...
export function Field({
  ...
  return (
    <label className="block">
      ...
      />
      {hint && <span className="block mt-1.5 text-[11px] text-slate-500 dark:text-white/40">{hint}</span>}
    </label>
  );
}
