...
import { Area, Field, Notice, PriorityBadge, PriorityPicker, SectionCard, Select, Stamp, priorityRank } from './parts';

type Counts = { comments: number; notes: number; links: number; drive: number; images: number };

const STARTERS = [
  { title: 'Learn GitHub', category: 'Learning', priority: 'downtime' as WorkstationPriority, summary: 'Repos, branches, pull requests and how we review each other code.' },
  { title: 'Learn Vercel', category: 'Tooling', priority: 'downtime' as WorkstationPriority, summary: 'Deploy previews, environment variables, domains and rollbacks.' },
  { title: 'Project onboarding', category: 'Project', priority: 'moderate' as WorkstationPriority, summary: 'Everything a teammate needs before touching a client project.' },
];
...
export default function SubjectList({
  ...
  const counts = useMemo(() => {
    ...
    entries.forEach((entry) => {
      ...
      if (!bucket) return;
      if (entry.kind === 'note') bucket.notes += 1; else bucket.comments += 1;
    });
    links.forEach((link) => {
      const bucket = map[link.subjectId];
      if (!bucket) return;
      if (link.kind === 'drive') bucket.drive += 1; else bucket.links += 1;
    });
  ...
  const tally = (subject: WorkstationSubject) => counts[subject.id] ?? { comments: 0, notes: 0, links: 0, drive: 0, images: 0 };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <button
          type="button"
          onClick={() => { setShowForm((current) => !current); setLocalError(''); }}
      ...
      <div className="flex flex-wrap gap-1.5">
        {(['all', ...PRIORITY_OPTIONS.map((option) => option.id)] as ('all' | WorkstationPriority)[]).map((id) => {
          const label = id === 'all' ? 'All subjects' : PRIORITY_OPTIONS.find((option) => option.id === id)!.short;
          const total = id === 'all'
            ? subjects.length
      ...
      {showForm && (
        <SectionCard title="New subject" icon={FolderOpen}>
          <div className="space-y-3">
            <Field label="Subject title" value={title} onChange={setTitle} placeholder="Learn GitHub" />
            ...
            <Area label="What is this about?" value={summary} onChange={setSummary} rows={3} placeholder="What the team should know or learn from this subject." />
            <div>
      ...
      {!canPost && (
        <Notice tone="info">Add your display name above to post subjects, comments, links and images.</Notice>
      )}
          ...
          {visible.map((subject) => {
            ...
            return (
              ...
              >
                ...
                {subject.summary && (
                  <p className="brand-copy text-xs leading-relaxed mt-2 line-clamp-2">{subject.summary}</p>
                )}

                <div className="flex items-center gap-3 mt-3 flex-wrap text-[11px] text-slate-500 dark:text-white/40">
                  <span className="inline-flex items-center gap-1"><StickyNote className="w-3.5 h-3.5" />{stat.notes}</span>
                  <span className="inline-flex items-center gap-1"><MessageSquare className="w-3.5 h-3.5" />{stat.comments}</span>
                  <span className="inline-flex items-center gap-1"><Link2 className="w-3.5 h-3.5" />{stat.links}</span>
                  <span className="inline-flex items-center gap-1"><HardDrive className="w-3.5 h-3.5" />{stat.drive}</span>
                  <span className="inline-flex items-center gap-1"><ImagePlus className="w-3.5 h-3.5" />{stat.images}</span>
      ...
      <p className="text-[11px] text-slate-400 dark:text-white/25 inline-flex items-center gap-1.5">
        <ChevronDown className="w-3 h-3" />Urgent subjects float to the top.
      </p>
    </div>
  );
}
