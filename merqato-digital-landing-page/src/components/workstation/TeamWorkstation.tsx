...
export default function TeamWorkstation() {
  ...
  const attachments = (snapshot?.attachments ?? []).filter((attachment) => attachment.subjectId === openId);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="brand-copy text-[11px] leading-relaxed mt-0.5">
            Subjects for the team to study — projects, GitHub, Vercel, anything worth learning together.
          </p>
        </div>
        <span
          className={cn(
          ...
          title={isCloud ? 'Synced to Supabase for the whole team' : 'No Supabase project configured — saved in this browser only'}
        >
      ...
      {!hasAuthorName() && (
        <Notice tone="info">
          Add your display name first — every comment, link and image gets stamped with it, and it is the only way
          the team knows who posted what.
        </Notice>
      )}
      ...
      {loading ? (
        <div className="space-y-2.5">
          {[0, 1, 2].map((row) => (
            <div key={row} className="h-24 rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-100/60 dark:bg-white/[0.04] animate-pulse" />
          ))}
        </div>
      ) : subject ? (
        <SubjectDetail
          ...
          onPriority={(priority: WorkstationPriority) => run(() => updateSubject(subject.id, { priority }))}
          onDeleteSubject={async () => {
            if (!window.confirm(`Delete "${subject.title}" and everything posted under it?`)) return;
            await run(async () => {
          ...
          onAddEntry={(kind, body, priority) => run(() => addEntry({ subjectId: subject.id, kind, body, priority }))}
          onDeleteEntry={(id) => run(() => deleteEntry(id))}
          onAddLink={(kind, url, label) => run(() => addLink({ subjectId: subject.id, kind, url, label }))}
      ...
      {!loading && !subject && (snapshot?.subjects.length ?? 0) > 0 && (
        <p className="text-[11px] text-slate-400 dark:text-white/25 inline-flex items-center gap-1.5">
          ...
          {(snapshot?.subjects.length ?? 0)} subject{(snapshot?.subjects.length ?? 0) === 1 ? '' : 's'} in the library.
        </p>
      )}
    </div>
  );
