...
export const PRIORITY_OPTIONS: {
  ...
  { id: 'downtime', label: 'Check when you have downtime', short: 'Downtime', hint: 'Study material — no deadline' },
];
...
function writeLocal(snapshot: WorkstationSnapshot) {
  localStorage.setItem(WORKSTATION_STORAGE_KEY, JSON.stringify({
...
function cloudUrl(path: string): string | undefined {
  ...
  try {
    return supabase.storage.from(WORKSTATION_BUCKET).getPublicUrl(path).data.publicUrl;
  } catch {
    return undefined;
  }
}
...
export async function loadWorkstation(): Promise<WorkstationSnapshot> {
  if (!isSupabaseConfigured || !supabase) {
    ...
    await Promise.all(snapshot.attachments.map(async (attachment) => {
...
async function uploadImage(file: File): Promise<string> {
  ...
  if (!isSupabaseConfigured || !supabase) return `${LOCAL_PREFIX}${await saveLocalAsset(file)}`;

  const path = `${token}/${fileName}`;
  const { error } = await requireClient().storage.from(WORKSTATION_BUCKET).upload(path, file, {
...
export async function createSubject(input: NewSubjectInput): Promise<void> {
  ...
  if (!isSupabaseConfigured) {
    mutateLocal((snapshot) => ({
      ...snapshot,
      subjects: [{
        id, title, summary: input.summary.trim(), category: input.category,
        priority: input.priority, coverPath, createdBy: who.name, authorToken: who.token,
  ...
  const { error } = await requireClient().from('workstation_subjects').insert({
    id,
    title,
...
export async function updateSubject(
  id: string,
  patch: Partial<Pick<WorkstationSubject, 'title' | 'summary' | 'category' | 'priority' | 'coverPath'>>,
): Promise<void> {
  if (!isSupabaseConfigured) {
    mutateLocal((snapshot) => ({
      ...snapshot,
      subjects: snapshot.subjects.map((subject) => subject.id === id
...
export async function addEntry(input: {
  ...
  if (!isSupabaseConfigured) {
    mutateLocal((snapshot) => ({
      ...snapshot,
      entries: [...snapshot.entries, {
        id, subjectId: input.subjectId, kind: input.kind, body,
        priority: input.kind === 'note' ? input.priority ?? 'moderate' : null,
        createdBy: who.name, authorToken: who.token, createdAt: now,
      }],
    }));
    return;
  }

  const { error } = await requireClient().from('workstation_entries').insert({
    id,
...
export async function deleteEntry(id: string): Promise<void> {
  if (!isSupabaseConfigured) {
    mutateLocal((snapshot) => ({ ...snapshot, entries: snapshot.entries.filter((entry) => entry.id !== id) }));
    return;
  }
...
export async function addLink(input: {
  ...
  if (!isSupabaseConfigured) {
    mutateLocal((snapshot) => ({
      ...snapshot,
      links: [...snapshot.links, {
        id, subjectId: input.subjectId, kind: input.kind, url: withScheme,
        label: input.label.trim(), createdBy: who.name, authorToken: who.token, createdAt: now,
      }],
    }));
    return;
  }

  const { error } = await requireClient().from('workstation_links').insert({
    id,
    subject_id: input.subjectId,
    kind: input.kind,
    url: withScheme,
...
export async function deleteLink(id: string): Promise<void> {
  if (!isSupabaseConfigured) {
    mutateLocal((snapshot) => ({ ...snapshot, links: snapshot.links.filter((link) => link.id !== id) }));
    return;
  }
  ...
  for (const file of files) {
    ...
    if (!isSupabaseConfigured) {
      mutateLocal((snapshot) => ({
        ...snapshot,
        attachments: [...snapshot.attachments, {
          id, subjectId, storagePath, fileName: file.name, contentType: file.type,
          sizeBytes: file.size, createdBy: who.name, authorToken: who.token, createdAt: now,
        }],
      }));
      continue;
    }
...
export async function deleteAttachment(id: string, storagePath: string): Promise<void> {
  if (!isSupabaseConfigured) {
    ...
    if (storagePath.startsWith(LOCAL_PREFIX)) await dropLocalAsset(storagePath.slice(LOCAL_PREFIX.length));
    return;
  }
